...
class Overwatch(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.api_url = "https://ow-api.com/v1/stats/pc/us/{battletag}/{type}/{heroes}"
    ...
    async def overwatch_base(self, ctx, member, hero=""):
        ...
        try:
            ...
            await ctx.send(f"Unknown BattleTag for member {member}. :frowning:")
            raise e
        ...
        async with self.bot.http_session.get(self.api_url.format(**payload)) as resp:
            try:
                resp.raise_for_status()
        ...
        for img in imgs:
            async with self.bot.http_session.get(
                data[img].strip()
            ) as resp:  # remove possible spaces
                if resp.status == 200:
    ...
    async def overwatch_quickplay(self, ctx, member: discord.Member = None):
        ...
        e.add_field(
            name="Medals",
            value=(
                f"{medB}:third_place: {medS}:second_place: "
    ...
    def _make_overwatch_profile_figure(self, data):
        ...
        filter = Image.open("cogs/Overwatch/portraitFilter.png").convert("RGBA")
        # create empty canvas
